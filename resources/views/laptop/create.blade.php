...
<head>
    ...
    <title>Tambah Laptop Baru</title>
    ...
    <div class="container mx-auto px-4 py-10 max-w-3xl">
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100">
            ...
            <div class="bg-gray-50 px-8 py-6 border-b border-gray-200 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Tambah Laptop Baru</h1>
                    <p class="text-sm text-gray-500 mt-1">Masukkan data inventaris laptop baru.</p>
                </div>
            ...
            <form action="{{ route('laptop.store') }}" method="POST" class="px-8 py-8 space-y-6">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Kode Laptop</label>
                        <input type="text" name="kode_laptop" value="{{ old('kode_laptop') }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white">
                            <option value="available" {{ old('status') == 'available' ? 'selected' : '' }}>Available (Bisa Disewa)</option>
                            <option value="disewa" {{ old('status') == 'disewa' ? 'selected' : '' }}>Disewa (Sedang Dipakai)</option>
                            <option value="tidak disewakan" {{ old('status') == 'tidak disewakan' ? 'selected' : '' }}>Tidak Disewakan (Rusak/Maintenance)</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="spesifikasi" rows="4" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                            placeholder="Processor, RAM, Storage, dll..." required>{{ old('spesifikasi') }}</textarea>
                    </div>
                ...
                <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-medium shadow-lg hover:shadow-indigo-500/30 transition-all transform hover:-translate-y-0.5">
                        <i class="fa-regular fa-floppy-disk mr-2"></i> Simpan Data Baru
                    </button>
                </div>
